...
'''
    1.将原有标记的42776条数据归类，从1-9，并将每个类别内部数据shuffle乱序，存储到一个csv中，类内无序，类外有序
'''
...
#
# print(new_data_csv.shape)  # (42776, 104)
#
# new_data_csv.to_csv('../dataset/CNN_band_label_42776.csv', header=False, index=False)

'''
    2.从上面分类后的数据集中抽取9个类别训练数据，每个类别200条，一共1800条数据，每条数据103通道信息+label，
    一共(1800,104)，以及测试数据，每个类别除去训练集200条，剩余的条数作为该类别的测试数据，一共(N,104)
    Train和Test两个数据集分别存储成csv格式，方便读取。
    训练集---../dataset/CNN_train_1800.csv
    测试集---../dataset/CNN_test_40976.csv
'''

# # 导入上面分类好有序的42776条数据
# data = pd.read_csv('../dataset/CNN_band_label_42776.csv', header=None)
# data = data.as_matrix()
#
# # 初始话写入csv的数据列表
# data_train_csv = list()
...
#
# print(new_data_train_csv.shape)  # (1800, 104)
#
# new_data_train_csv.to_csv('../dataset/CNN_train_1800.csv', header=False, index=False)
#
# new_data_test_csv = pd.DataFrame(data_test_csv)
#
# print(new_data_test_csv.shape)  # (40976, 104) = 42776-1800
#
# new_data_test_csv.to_csv('../dataset/CNN_test_40976.csv', header=False, index=False)

'''
    3.将train和test数据集shuffle打乱顺序
    训练集---../dataset/CNN_train_1800_shuffle.csv
    测试集---../dataset/CNN_test_40976_shuffle.csv
'''

# # train数据集
# train_data = pd.read_csv('../dataset/CNN_train_1800.csv', header=None)
#
...
#
# print(new_train_list.shape)  # (1800, 104)
#
# new_train_list.to_csv('../dataset/CNN_train_1800_shuffle.csv', header=False, index=False)
#
# # test数据集
# test_data = pd.read_csv('../dataset/CNN_test_40976.csv', header=None)
#
...
#
# print(new_test_list.shape)  # (40976, 104)
#
# new_test_list.to_csv('../dataset/CNN_test_40976_shuffle.csv', header=False, index=False)

'''
    4.将train和test标签转化为onehot矩阵格式，并存储
    ---../dataset/CNN_train_1800_shuffle_label_onehot.csv
    ---../dataset/CNN_test_40976_shuffle_label_onehot.csv
'''
...
# # train数据集
# train_data = pd.read_csv('../dataset/CNN_train_1800_shuffle.csv', header=None)
# train_data = train_data.as_matrix()
#
# # 获取特征矩阵
...
#
# onehot_csv.to_csv('../dataset/CNN_train_1800_shuffle_label_onehot.csv', header=False, index=False)
#
# # test数据集
# test_data = pd.read_csv('../dataset/CNN_test_40976_shuffle.csv', header=None)
# test_data = test_data.as_matrix()
#
# # 获取特征矩阵
# data_content = test_data[:, :-1]
# print(data_content.shape)  # (40976, 103)
#
# # 获取标记矩阵
...
# onehot_csv = pd.DataFrame(onehot_csv)
#
# onehot_csv.to_csv('../dataset/CNN_test_40976_shuffle_label_onehot.csv', header=False, index=False)

...
'''
    在CNN_1D.py的基础上，不单对单一包围像素进行更新分类，
...
# 加载SVM模型
model = joblib.load('../SVM/models/SVC.m')
...
# 获取错误分类坐标
wrong_loc = return_wrong(pred, temp_list)
# 临时保存wrong_loc
temp_wrong_loc = return_wrong(pred, pavia_gt_label)

print(len(wrong_loc))  # 7112
...
def mani(loc, flag):
    ...
    classes = set()
    # 统计边缘集合中像素类别是否一致：首先看是否都预测正确，在这个前提下，查看统一的类别序号
    ...
    # 如果边缘集合元素判定正误都属于一类，则说明该连通区域位于标记数据集内部，并协同调整内部标记
    if len(classes) == 1:
...
print('新正确率：', float(new_acc) * 100, '%')  # 新正确率：
...
#
# # 存储图片
# cv2.imwrite('../imgs/SVM_Revo_2-1.png', single_merged)
#
# # 将更正的分类坐标重新用黄色标记
# for value in change_list:
#     j = int(value % 340)
...
#
# # 存储图片
# cv2.imwrite('../imgs/SVM_Revo_2-2.png', single_merged)

...
lle = manifold.LocallyLinearEmbedding(n_components=49)

X = lle.fit_transform(data_content)

print(X.shape)

# # PCA降维获取49个通道
# pca = PCA(n_components=49)
# data_content = pca.fit_transform(data_content)
# print(data_content.shape)  # (42776, 49)
...
#
# print(new_data_csv.shape)  # (42776, 50)
#
# new_data_csv.to_csv('./dataset/CNN_PCA/CNN_data.csv', header=False, index=False)

'''
    2.从上面分类后的数据集中抽取9个类别训练数据，每个类别200条，一共1800条数据，每条数据49通道信息+label，
    一共(1800,50)，以及测试数据，每个类别除去测试集200条，剩余的条数作为该类别的测试数据，一共(N,50)
    Train和Test两个数据集分别存储成csv格式，方便读取。
    ---./dataset/CNN_PCA/CNN_train_data.csv
    ---./dataset/CNN_PCA/CNN_test_data.csv
'''

# # 导入上面分类好有序的42776条PCA降维数据
# data = pd.read_csv('./dataset/CNN_PCA/CNN_data.csv', header=None)
# data = data.as_matrix()
#
# # 初始话写入csv的数据列表
# data_train_csv = list()
...
#
# print(new_data_train_csv.shape)  # (1800, 50)
#
# new_data_train_csv.to_csv('./dataset/CNN_PCA/CNN_train_data.csv', header=False, index=False)
#
# new_data_test_csv = pd.DataFrame(data_test_csv)
#
# print(new_data_test_csv.shape)  # (40976, 50) = 42776-1800
#
# new_data_test_csv.to_csv('./dataset/CNN_PCA/CNN_test_data.csv', header=False, index=False)

'''
    3.将train和test数据集shuffle打乱顺序
    ---./dataset/CNN_PCA/CNN_train_data_shuffle.csv
    ---./dataset/CNN_PCA/CNN_test_data_shuffle.csv
'''

# # train数据集
# train_data = pd.read_csv('./dataset/CNN_PCA/CNN_train_data.csv', header=None)
#
...
#
# print(new_train_list.shape)  # (1800, 50)
#
# new_train_list.to_csv('./dataset/CNN_PCA/CNN_train_data_shuffle.csv', header=False, index=False)
#
# # test数据集
# test_data = pd.read_csv('./dataset/CNN_PCA/CNN_test_data.csv', header=None)
#
...
#
# print(new_test_list.shape)  # (40976, 50)
#
# new_test_list.to_csv('./dataset/CNN_PCA/CNN_test_data_shuffle.csv', header=False, index=False)

'''
    4.将train和test标签转化为onehot矩阵格式，并存储
    ---./dataset/CNN_PCA/CNN_train_data_shuffle_label_onehot.csv
    ---./dataset/CNN_PCA/CNN_test_data_shuffle_label_onehot.csv
'''
...
# # train数据集
# train_data = pd.read_csv('./dataset/CNN_PCA/CNN_train_data_shuffle.csv', header=None)
# train_data = train_data.as_matrix()
#
# # 获取特征矩阵
# data_content = train_data[:, :-1]
# print(data_content.shape)  # (1800, 50)
#
# # 获取标记矩阵
...
#
# onehot_csv.to_csv('./dataset/CNN_PCA/CNN_train_data_shuffle_label_onehot.csv', header=False, index=False)
#
# # test数据集
# test_data = pd.read_csv('./dataset/CNN_PCA/CNN_test_data_shuffle.csv', header=None)
# test_data = test_data.as_matrix()
#
# # 获取特征矩阵
# data_content = test_data[:, :-1]
# print(data_content.shape)  # (40976, 49)
#
# # 获取标记矩阵
...
# onehot_csv = pd.DataFrame(onehot_csv)
#
# onehot_csv.to_csv('./dataset/CNN_PCA/CNN_test_data_shuffle_label_onehot.csv', header=False, index=False)
...
#
# # 验证尺寸
# print(data_band_batch_100.shape)  # (100, 49)
